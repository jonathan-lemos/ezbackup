...
struct options{
	...
	union tagflags{                         /**< The special flags to use. This can be represented as a series of bits or as an unsigned integer. */
		...
		unsigned          dword;            /**< All flags together. */
	}flags;
};
 ...
 *
 * @return A new options structure, or NULL on failure.
 ...
 * Returns 0 if they are the same and non-zero if they are different.
 ...
 *
 * @param o The options structure to free.
 ...
 *
 * @param out A pointer to an options structure that should be filled.
 * This cannot be NULL, but can point to NULL.
 * If it does not point to NULL, the existing structure will be freed.
 * If this function fails, the structure will be set to NULL.
 * This structure must be freed when no longer in use.
 *
 * @param op_out A pointer to an enum OPERATION that corresponds to the operation that the user selects.
 ...
 * This file must be created by write_options_tofile()
 ...
 * The contents can be parsed with parse_options_fromfile()
 ...
 *
 * @param file Path to the destination of the options file.
 ...
 * This function is valid even when the program exits and reopens, as long as the file created by set_prev_options() still exists.
 ...
 * @see get_prev_options()
 *
 * @param opt The options file to write to disk.
