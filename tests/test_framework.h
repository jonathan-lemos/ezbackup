...
enum TEST_STATUS{
	...
	TEST_FAIL = 1     /**< The test failed. */
};

/**
 * @brief Logs a message to stderr in red.
...
/**
 * @brief Logs a message to stderr in yellow.
 * If stderr does not point to a tty, the color is not used.
 *
 * @param format A printf format string.
 *
...
/**
 * @brief Logs a message to stderr in green.
 * If stderr does not point to a tty, the color is not used.
 *
 * @param format A printf format string.
 *
...
/**
 * @brief Logs a message to stderr in blue.
 * If stderr does not point to a tty, the color is not used.
 *
 * @param format A printf format string.
 *
...
/**
 * @brief Creates a file with the specified path and data.
 * An internal error is thrown and the program exits if there is an error.
 ...
 *
 * @return 0 if the contents of the file and data are equivalent, negative if the first character that doesn't match is less in the file, positive if it is greater.
 ...
 *
 * @return 0 if the contents of the two files are equivalent, negative if the first character that doesn't match is less in file1, positive if it is greater.
...
/**
 * @brief Sets up a basic test environment.
 * It will have the following structure:
 * path (0755)
 *     path/file_{00-20}.txt (0666)
 *
 * This environment should be cleaned up with cleanup_test_environment() when it is no loner needed.
...
/**
 * @brief Sets up a complex test environment.
 * It will have the following structure:
 * path (0755)
 * 		path/dir1 (0755)
 ...
 *
 * @param files The filename array generated by a previous call to setup_test_environment_*()
...
/**
 * @brief Fills a buffer with sample data.
...
/**
 * @brief Makes a path out of a variable number of components.
 * Examples:
 ...
 *
 * @return A string containing the created path.
 * This string must be free()'d when no longer in use.
 */
char* make_path(int n_components, ...);

/**
 * @brief Returns 0 if the condition is true. Otherwise prints an "Assertion Failed" message and returns non-zero.
 * Do not call this function directly; use the TEST_ASSERT() macro.
 * @see TEST_ASSERT()
 *
 * @param condition The condition to verify.
 *
 * @param file The name of the file the assertion is being called from.
 * This argument should be filled with the __FILE__ macro.
 *
 * @param line The line number the assertion is being called from.
...
/**
 * @brief Does a compile-time assertion. The expression must be evaluated at compile-time.
 * This expression works because if the condition is true, then sizeof(char[1 - 0]) is a valid expression.
...
/**
 * @brief Does a runtime assertion.
 * If the condition is false, a message is displayed to the user showing the file, line number, and the failing condition as a string. The test immediately starts cleaning up afterwards.
 * This expression can only be called within a function that matches the prototype void(*)(enum TEST_STATUS status).
 * This parameter must be named "status"
 * There also must be a label "cleanup" that can be goto'd.
 *
 * These are necessary to allow the test to report failure while also cleaning up any dynamically allocated resources before exiting.
 *
 * @param condition The condition to verify.
 */
...
/**
 * @brief Does a runtime assertion and displays a custom message.
 * If the condition is false, a message is displayed to the user showing the file, line number, and a custom message. The test immediately starts cleaning up afterwards.
 * This expression can only be called within a function that matches the prototype void(*)(enum TEST_STATUS status).
 * This parameter must be named "status"
 * There also must be a label "cleanup" that can be goto'd.
 *
...
/**
 * @brief Frees a resource and sets it to NULL.
 * This is necessary to prevent double free problems arising from the cleanup label.
 * Example usage:
 ...
 *
 * @param free_func The function used to free.
...
/**
 * @brief Frees a resource and sets it to NULL on success or failure. On failure it fails a test assertion.
 * This is necessary to prevent double free problems arising from the cleanup label.
 * If the free function's return value does not equal zero, a message is displayed to the user showing the file, line number, and the free function that failed. The test immediately starts cleaning up afterwards.
 * This expression can only be called within a function that matches the prototype void(*)(enum TEST_STATUS status).
 * This parameter must be named "status"
 * There also must be a label "cleanup" that can be goto'd.
 *
...
struct unit_test{
	/**
	 * @brief The unit test's function.
	 * This function must fill the following prototype:
	 * void(*)(enum TEST_STATUS* status)
	 * The parameter must be named "status".
	 *
	 * Do not edit the value this parameter points to directly; use the TEST_ASSERT()/TEST_ASSERT_MSG() macros to indicate failure.
	 * This value will be TEST_SUCCESS by default, so no additional action is needed to indicate success.
	 * @see TEST_ASSERT()
	 * @see TEST_ASSERT_MSG()
	 *
	 * If you want to use the TEST_ASSERT()/TEST_ASSERT_MSG() macros, there must be a label "cleanup" that can be goto'd.
...
/**
 * @brief Turns a unit test function into a struct unit_test that does not require user input.
 * This is designed to be used with a struct unit_test array like below:
...
/**
 * @brief Turns a unit test function into a struct unit_test that does require user input.
 * This is designed to be used with a struct unit_test array like below:
 * const struct unit_test tests[] = {
...
/**
 * @brief A package containing multiple unit tests.
 * Do not initialize these directly. Use the MAKE_PKG() macro.
 *
 * @see MAKE_PKG()
 */
struct test_pkg{
	const struct unit_test* tests; /**< The array of tests. */
	const size_t tests_len;        /**< The number of tests in the array. */
	const char* name;              /**< The name of the test package. */
};

/**
 * @brief Converts a unit test array into a unit test package.
 * This must be called under the same scope as the test array in question.
 * The package can be used like any other variable under the pkg_name specified.
 * To export the package for use with another file, use the EXPORT_PKG() macro in a header file.
 * @see EXPORT_PKG()
 *
 * @param tests The array of tests to convert.
...
/**
 * @brief Exports a package made with MAKE_PKG() for use with other files.
 * This must be used in a header file seperate from the package definition.
 * @see MAKE_PKG()
 *
 * @param export_name The name of the package to export.
 * This is the second parameter to the MAKE_PKG() macro.
 */
#define EXPORT_PKG(export_name) extern const struct test_pkg export_name

#define RT_NORMAL         (0x0) /**< No special options. Only valid by itself. */
#define RT_NO_RU_TESTS    (0x1) /**< Exclude all tests that require the user. */
#define RT_NO_NONRU_TESTS (0x2) /**< Exclude all tests that don't require the user. */

/**
 * @brief Executes an array of packages.
